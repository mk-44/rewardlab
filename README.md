...
```bash
python -m rlhf audit --config data/preference_data/rhyme_failed/data.yaml    # 10 seconds, no model, no GPU
```
...
[`data/preference_data/rhyme_failed/NEGATIVE_RESULT.md`](data/preference_data/rhyme_failed/NEGATIVE_RESULT.md).
...
pip install -e ".[data,rhyme,dev]"
python -m rlhf --help          # or: rewardlab --help
```

All commands run **from the repository root**, because config paths are root-relative.
`pytest` needs no arguments; `pyproject.toml` points it at `tests/`.

## Quickstart

```bash
python -m rlhf resplit --config data/preference_data/rhyme_failed/data.yaml   # honest prompt-level split
python -m rlhf audit   --config data/preference_data/rhyme_failed/data.yaml   # Tier 0
python -m rlhf profile --config data/preference_data/rhyme_failed/data.yaml   # Tier 1
...
The DPO run has its own step list: [experiments/dpo/non_sft_50k_data/README.md](experiments/dpo/non_sft_50k_data/README.md).

## Layout

```
rlhf/                          the only importable package
  core/                        shared. never imports a method folder.
    contracts, device, logging
    preference/     schema, loaders, formats, splits, collate
    data_analysis/  audit (Tier 0), distribution (Tier 1)
    training/       optim, checkpoint
    policy/  scoring/  evaluate/
  reward/
    config
    model/          backbone, head, model, losses
    training/       trainer, evaluate, metrics
    analysis/       diagnostics, ledger
    cli/            main
  dpo/  ppo/  grpo/            reserved

experiments/reward/            configs, ledgers and run reports; weights ignored
data/preference_data/          rhyme_50k, rhyme_10k, rhyme_failed
scripts/                       reproducible dataset generation
tests/                         mirrors rlhf/, 1178 tests
```

`rlhf/core/` never imports a method folder, so the dependency runs one way only. Code
moves up into `core/` when a second method actually needs it, not before. Generated
artifacts live outside the package, so nothing under `rlhf/` is written at runtime.
