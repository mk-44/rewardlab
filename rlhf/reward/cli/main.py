...
def _build_collator(cfg: AppConfig):
    from rlhf.core.preference.collate import PairCollator
...
def _build_store(cfg: AppConfig, path: str, collator):
    from rlhf.core.preference.collate import PretokenizedPairs
...
def cmd_resplit(cfg : AppConfig, args) -> int:
    from rlhf.core.preference.splits import resplit_files
...
def cmd_audit(cfg: AppConfig, args) -> int:
    from rlhf.core.data_analysis.audit import audit, render
...
def cmd_profile(cfg : AppConfig, args) -> int:
    from rlhf.core.data_analysis.distribution import profile, render
    train_groups, _ = _load_pair_views(cfg, cfg.data.train_path)
    val_groups = None
...
def cmd_train(cfg: AppConfig, args) -> int:
    ...
    from rlhf.reward.training.trainer import Trainer
...
def cmd_ledger(cfg: AppConfig, args) -> int:
    from rlhf.reward.analysis.ledger import build_ledger, check_shared_space, render_ledger
